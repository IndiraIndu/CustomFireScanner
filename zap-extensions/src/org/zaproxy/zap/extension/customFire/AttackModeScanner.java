...
public class AttackModeScanner implements EventConsumer {
	...
	public AttackModeScanner(ExtensionCustomFire extension) {
	...
	private class AttackModeThread implements Runnable, ScannerListener {
		...
		private CustomScan cscanWrapper;
		...
		public void run() {
			...
			this.running = true;
			cscanWrapper = new CustomScan(Constant.messages.getString("ascan.attack.scan"), extension.getScannerParam(), 
					extension.getModel().getOptionsParam().getConnectionParam(), extension.getPolicyManager().getAttackScanPolicy());
			extension.registerScan(cscanWrapper);
			while (running) {
		...
		public void notifyNewMessage(HttpMessage msg) {
			cscanWrapper.notifyNewMessage(msg);
		}
