 ...
 */
public abstract class ScanPanel<GS extends GenericScanner2, SC extends ScanController<GS>> extends AbstractPanel {
	 ...
	 * @param scanParam
	 */
	public ScanPanel(String prefix, ImageIcon icon, SC controller, AbstractParam scanParam) {
		super();
		this.prefix = prefix;
		this.controller = controller;
		selectScanEntry = new ScanEntry<>(Constant.messages.getString("customFire.custom.toolbar.progress.select"));
		initialize(icon);
	...
	private  void initialize(ImageIcon icon) {
		...
		this.setName(Constant.messages.getString("customFire.custom.panel.title"));
		this.setIcon(icon);
		this.add(getPanelCommand(), "customFire.custom.panel");
		scanStatus = new ScanStatus(icon, Constant.messages.getString("customFire.custom.panel.title"));
	...
	private javax.swing.JPanel getPanelCommand() {
		if (panelCommand == null) {
			...
			panelCommand.setName("customFire.custom.panel");
	...
	private javax.swing.JToolBar getPanelToolbar() {
		if (panelToolbar == null) {
			...
			panelToolbar.setName("customFire.custom.toolbar");
			...
			panelToolbar.add(new JLabel(Constant.messages.getString("customFire.custom.toolbar.progress.label")), getGBC(x++,0));
			...
			panelToolbar.add(getCustomScansNameLabel(), getGBC(x++,0));
			panelToolbar.add(getCustomScansValueLabel(), getGBC(x++,0));
	...
	private JLabel getCustomScansNameLabel() {
		if (scannedCountNameLabel == null) {
			scannedCountNameLabel = new javax.swing.JLabel();
			scannedCountNameLabel.setText(Constant.messages.getString("customFire.custom.toolbar.cscans.label"));
		}
		return scannedCountNameLabel;
	}

	private JLabel getCustomScansValueLabel() {
	...
	private void setCustomScanLabelsEventHandler() {
		List<GS> cscans = controller.getActiveScans();
		getCustomScansValueLabel().setText(String.valueOf(cscans.size()));
		StringBuilder sb = new StringBuilder();
		sb.append("<html>");
		for (GS cscan : cscans) {
			sb.append(cscan.getDisplayName());
			sb.append("<br>");
		}
		sb.append("</html>");

		final String toolTip = sb.toString();
		getCustomScansNameLabel().setToolTipText(toolTip);
		getCustomScansValueLabel().setToolTipText(toolTip);

		scanStatus.setScanCount(cscans.size());
		this.getClearScansButton().setEnabled(controller.getAllScans().size() - cscans.size() > 0);
	...
	protected JButton getStopScanButton() {
		if (stopScanButton == null) {
			...
			stopScanButton.setToolTipText(Constant.messages.getString("customFire.custom.toolbar.button.stop"));
			stopScanButton.setIcon(new ImageIcon(ScanPanel.class.getResource("/org/zaproxy/zap/extension/customFire/resources/142.png")));
	...
	protected JToggleButton getPauseScanButton() {
		if (pauseScanButton == null) {
			...
			pauseScanButton.setToolTipText(Constant.messages.getString("customFire.custom.toolbar.button.pause"));
			pauseScanButton.setSelectedToolTipText(Constant.messages.getString("customFire.custom.toolbar.button.unpause"));
	...
	private JButton getOptionsButton() {
		if (optionsButton == null) {
			...
			optionsButton.setToolTipText(Constant.messages.getString("customFire.custom.toolbar.button.options"));
			...
			optionsButton.addActionListener(new ActionListener () {
				...
				public void actionPerformed(ActionEvent e) {
					Control.getSingleton().getMenuToolsControl().options(
							Constant.messages.getString("customFire.custom.options.title"));
				}
			});
		}
	...
	private JButton getClearScansButton() {
		if (clearScansButton == null) {
			...
			clearScansButton.setToolTipText(Constant.messages.getString("customFire.custom.toolbar.button.clear"));
			clearScansButton.setIcon(new ImageIcon(ScanPanel.class.getResource("/resource/icon/fugue/broom.png")));
	...
	private void scanFinshedEventHandler(int id, String host) {
		...
		setCustomScanLabelsEventHandler();
	}
