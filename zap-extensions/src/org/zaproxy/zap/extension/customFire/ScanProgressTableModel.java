...
public class ScanProgressTableModel extends AbstractTableModel {
	...
	private static final String[] columnNames = {//"Plugin",
			"name",
			"strength",
			"progress",
			"time",
	...
	public Object getValueAt(int row, int col) {
		...
		if (row < values.size()) {
			...
			switch (col) {
			case 0:
				//return Constant.messages.getString("customfire.custom.progress.label.totals");                    
				return "Totals";
			case 3:
				return totTime;
	 ...
	 * @param scan 
	 */
	public void updateValues(CustomScan scan, HostProcess hp) {
		values.clear();
