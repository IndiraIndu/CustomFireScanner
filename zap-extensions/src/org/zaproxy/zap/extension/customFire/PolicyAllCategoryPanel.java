...
import org.zaproxy.zap.extension.customFire.AllCategoryTableModel;
...
public class PolicyAllCategoryPanel extends AbstractParamPanel {
	...
	private CustomScanPolicy policy;
	 ...
	 * @param policy
	 */
	public PolicyAllCategoryPanel(Window parent, ExtensionCustomFire extension, CustomScanPolicy policy) {
	 ...
	 * @param switchable
	 */
	public PolicyAllCategoryPanel(Window parent, ExtensionCustomFire extension, CustomScanPolicy policy, boolean switchable) {
		super();
	...
	private void initialize() {
		...
		int row = 0;
		this.add(new JLabel(Constant.messages.getString("customFire.custom.policy.name.label")),
		...
		row++;
		this.add(new JLabel(Constant.messages.getString("customFire.custom.options.level.label")),
				LayoutHelper.getGBC(0, row, 1, 0.0D, 0, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2)));
		...
		row++;
		this.add(new JLabel(Constant.messages.getString("customFire.custom.options.strength.label")),
				LayoutHelper.getGBC(0, row, 1, 0.0D, 0, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2)));
		...
		applyToPanel.add(new JLabel(Constant.messages.getString("customFire.custom.options.apply.label")), 
				LayoutHelper.getGBC(0, 0, 1, 0.0, new Insets(2, 2, 2, 2)));
		applyToPanel.add(getApplyToThreshold(), LayoutHelper.getGBC(1, 0, 1, 0.0));
		applyToPanel.add(new JLabel(Constant.messages.getString("customFire.custom.options.thresholdTo.label")), 
				LayoutHelper.getGBC(2, 0, 1, 0.0, new Insets(2, 2, 2, 2)));
		applyToPanel.add(getApplyToThresholdTarget(), LayoutHelper.getGBC(3, 0, 1, 0.0));
		applyToPanel.add(new JLabel(Constant.messages.getString("customFire.custom.options.rules.label")), LayoutHelper.getGBC(4, 0, 1, 0.0, new Insets(2, 2, 2, 2)));
		JButton applyThresholdButton = new JButton(Constant.messages.getString("customFire.custom.options.go.button"));
		...
		applyToPanel.add(new JLabel(Constant.messages.getString("customFire.custom.options.strengthTo.label")), LayoutHelper.getGBC(2, 1, 1, 0.0, new Insets(2, 2, 2, 2)));
		applyToPanel.add(getApplyToStrengthTarget(), LayoutHelper.getGBC(3, 1, 1, 0.0));
		applyToPanel.add(new JLabel(Constant.messages.getString("customFire.custom.options.rules.label")), LayoutHelper.getGBC(4, 1, 1, 0.0, new Insets(2, 2, 2, 2)));
		JButton applyStrengthButton = new JButton(Constant.messages.getString("customFire.custom.options.go.button"));
	...
	public void initialise(CustomScanPolicy policy) {
	...
	/**
	 * Reloads the custom scan policies, which will pick any new ones that have been defined and selects the policy with the given
	 * name.
	 * 
	...
	private AlertThreshold strToThreshold(String str) {
		if (str.equals(Constant.messages.getString("customFire.custom.options.level.off"))) {
			return AlertThreshold.OFF;
		}
		if (str.equals(Constant.messages.getString("customFire.custom.options.level.low"))) {
			return AlertThreshold.LOW;
		}
		if (str.equals(Constant.messages.getString("customFire.custom.options.level.medium"))) {
			return AlertThreshold.MEDIUM;
		}
		if (str.equals(Constant.messages.getString("customFire.custom.options.level.high"))) {
	...
	private JComboBox<String> getApplyToThreshold() {
		if (applyToThreshold == null) {
			...
			applyToThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.default"));
			applyToThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.off"));
			applyToThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.low"));
			applyToThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.medium"));
			applyToThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.high"));
		}
	...
	private JComboBox<String> createStatusComboBox() {
		...
		comboBox.addItem(Constant.messages.getString("customFire.custom.policy.table.quality.all"));
	...
	private boolean hasSameStatus(Plugin scanner, String status) {
		if (status.equals(Constant.messages.getString("customFire.custom.policy.table.quality.all"))) {
			return true;
		}
	...
	private AttackStrength strToStrength(String str) {
		if (str.equals(Constant.messages.getString("customFire.custom.options.strength.low"))) {
			return AttackStrength.LOW;
		}
		if (str.equals(Constant.messages.getString("customFire.custom.options.strength.medium"))) {
			return AttackStrength.MEDIUM;
		}
		if (str.equals(Constant.messages.getString("customFire.custom.options.strength.high"))) {
			return AttackStrength.HIGH;
		}
		if (str.equals(Constant.messages.getString("customFire.custom.options.strength.insane"))) {
	...
	private JComboBox<String> getApplyToStrength() {
		if (applyToStrength == null) {
			...
			applyToStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.default"));
			applyToStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.low"));
			applyToStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.medium"));
			applyToStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.high"));
			applyToStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.insane"));
		}
	...
	private void setThreshold(AlertThreshold threshold) {
		getComboThreshold().setSelectedItem(
				Constant.messages.getString("customFire.custom.options.level." + threshold.name().toLowerCase()));
		getThresholdNotes().setText(
				Constant.messages.getString("customFire.custom.options.level." + threshold.name().toLowerCase() + ".label"));
	}

	private void setStrength(AttackStrength strength) {
		getComboStrength().setSelectedItem(
				Constant.messages.getString("customFire.custom.options.strength." + strength.name().toLowerCase()));
		getStrengthNotes().setText(
				Constant.messages.getString("customFire.custom.options.strength." + strength.name().toLowerCase() + ".label"));
	}
	...
	private JTable getTableTest() {
		if (tableTest == null) {
			...
			for (AlertThreshold level : AlertThreshold.values()) {
				jcb1.addItem(Constant.messages.getString("customFire.custom.policy.level." + level.name().toLowerCase()));
			}
			...
			for (AttackStrength level : AttackStrength.values()) {
				jcb2.addItem(Constant.messages.getString("customFire.custom.policy.level." + level.name().toLowerCase()));
			}
	...
	public void validateParam(Object obj) throws Exception {
		...
		if (newName.length() == 0) {
			throw new Exception(Constant.messages.getString("customFire.custom.policy.warn.noname"));
		} else if (! extension.getPolicyManager().isLegalPolicyName(newName)) {
			throw new Exception(Constant.messages.getString("customFire.custom.policy.warn.badname", PolicyManager.ILLEGAL_POLICY_NAME_CHRS));

		} else if (! newName.equals(currentName)) {
			// Name changed
			if (extension.getPolicyManager().getAllPolicyNames().contains(newName)) {
				throw new Exception(Constant.messages.getString("customFire.custom.policy.warn.exists"));
			}
		}
	}
	...
	private JComboBox<String> getComboThreshold() {
		if (comboThreshold == null) {
			...
			comboThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.low"));
			comboThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.medium"));
			comboThreshold.addItem(Constant.messages.getString("customFire.custom.options.level.high"));
			comboThreshold.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					// Set the explanation and save
					if (comboThreshold.getSelectedItem().equals(Constant.messages.getString("customFire.custom.options.level.low"))) {
						getThresholdNotes().setText(Constant.messages.getString("customFire.custom.options.level.low.label"));
						policy.setDefaultThreshold(AlertThreshold.LOW);

					} else if (comboThreshold.getSelectedItem().equals(Constant.messages.getString("customFire.custom.options.level.medium"))) {
						getThresholdNotes().setText(Constant.messages.getString("customFire.custom.options.level.medium.label"));
						policy.setDefaultThreshold(AlertThreshold.MEDIUM);

					} else {
						getThresholdNotes().setText(Constant.messages.getString("customFire.custom.options.level.high.label"));
	...
	private JComboBox<String> getComboStrength() {
		if (comboStrength == null) {
			...
			comboStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.low"));
			comboStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.medium"));
			comboStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.high"));
			comboStrength.addItem(Constant.messages.getString("customFire.custom.options.strength.insane"));
			comboStrength.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					// Set the explanation and save
					if (comboStrength.getSelectedItem().equals(Constant.messages.getString("customFire.custom.options.strength.low"))) {
						getStrengthNotes().setText(Constant.messages.getString("customFire.custom.options.strength.low.label"));
						policy.setDefaultStrength(AttackStrength.LOW);

					} else if (comboStrength.getSelectedItem().equals(Constant.messages.getString("customFire.custom.options.strength.medium"))) {
						getStrengthNotes().setText(Constant.messages.getString("customFire.custom.options.strength.medium.label"));
						policy.setDefaultStrength(AttackStrength.MEDIUM);

					} else if (comboStrength.getSelectedItem().equals(Constant.messages.getString("customFire.custom.options.strength.high"))) {
						getStrengthNotes().setText(Constant.messages.getString("customFire.custom.options.strength.high.label"));
						policy.setDefaultStrength(AttackStrength.HIGH);

					} else {
						getStrengthNotes().setText(Constant.messages.getString("customFire.custom.options.strength.insane.label"));
	...
	private void fireScanPolicyChanged(CustomScanPolicy scanPolicy) {
